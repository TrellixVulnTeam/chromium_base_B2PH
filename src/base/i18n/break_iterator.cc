...
bool BreakIterator::Init() {
  ...
  if (break_type_ == RULE_BASED) {
    iter_ = ubrk_openRules((const UChar*)rules_.c_str(),
                           static_cast<int32_t>(rules_.length()),
                           (const UChar*)string_.data(),
    ...
    iter_ = ubrk_open(break_type, nullptr, (const UChar*)string_.data(),
...
bool BreakIterator::SetText(const base::char16* text, const size_t length) {
  UErrorCode status = U_ZERO_ERROR;
  ubrk_setText(static_cast<UBreakIterator*>(iter_), (const UChar*)text, length,
               &status);
